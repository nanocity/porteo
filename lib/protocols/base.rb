    ...
    class Base
      ...
      def send_message
        ...
        begin
          ...
          @gateway = Porteo::Gateway.const_get( "#{@gw_config[:gateway]}".capitalize.to_sym ).new( @gw_config )
        rescue NameError
          raise ArgumentError, "Protocol Error. Undefined gateway. Check if Porteo::Gateway::#{@gw_config[:gateway].capitalize.to_sym} is a valid gateway"
        end
        # Send the message
      ...
      def expand_template
        ...
        @requires.each do |required_param|
          raise ArgumentError, "Protocol Error. Required parameter '#{required_param.to_s}' is not present or it is nil." unless @param[required_param]
        end

        # This variable will be used to eval the template
        param = @param
        erb_template = ERB.new( @template.to_s, 0, "%<>" )
